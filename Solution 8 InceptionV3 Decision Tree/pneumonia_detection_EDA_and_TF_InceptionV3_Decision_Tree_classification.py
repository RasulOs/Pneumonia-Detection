...
# Put 0 if you don't want to train any layers. 
# Put 1 if you want to train only the last layer.
# Put 2 if you want to train the last 2 layers and etc.
trainable_layers_count = 0

epoch_number = 2
batch_size_number = 16
learning_rate_number = 0.000001
...
for i, each_layer in enumerate(pretrained_model.layers):

    if trainable_layers_count > 0:
        if i < len(pretrained_model.layers) - trainable_layers_count:
...
# %%
# Compile the model
# specify learning rate as 0.00001
model.compile(optimizer=keras.optimizers.Adam(learning_rate=learning_rate_number), loss=tf.keras.losses.BinaryCrossentropy(), metrics=['accuracy'])
...
# %%
history_pretrained_model = model.fit(x=X_train, y=y_train, epochs=epoch_number, validation_data=(X_val, y_val), batch_size=batch_size_number)
...
plt.subplot(2, 2, 1)
plt.plot(history_pretrained_model.history['accuracy'], label='Train Accuracy')
plt.plot(history_pretrained_model.history['val_accuracy'], label='Validation Accuracy')
plt.legend(loc='lower right')
plt.title('Train and Validation Accuracy')

plt.subplot(2, 2, 2)
plt.plot(history_pretrained_model.history['loss'], label='Train Loss')
plt.plot(history_pretrained_model.history['val_loss'], label='Validation Loss')
...
# %%
# Take weights of the last layer and insert them into 
# a decision tree.
...
# %%
print(f"type of model.layers[-1].get_weights: {type(model.layers[-1].get_weights())}")
print(f"type of model.layers[-1].get_weights[0]: {type(model.layers[-1].get_weights()[0])}")
print(f"len of model.layers[-1].get_weights(): {len(model.layers[-1].get_weights())}")
print(f"shape of model.layers[-1].get_weights()[0] {model.layers[-1].get_weights()[0].shape}")
print(f"shape of model.layers[-1].get_weights()[1] {model.layers[-1].get_weights()[1].shape}")
print(f"model.layers[-1].get_weights: {model.layers[-1].get_weights()}")

# %%
weights_and_bias_from_last_layer = list()

for i in range(len(model.layers[-1].get_weights()[0])):
    weights_and_bias_from_last_layer.append(model.layers[-1].get_weights()[0][i][0])

for i in model.layers[-1].get_weights()[1]:
    weights_and_bias_from_last_layer.append(i)


print(f"len of weights_and_bias_from_last_layer: {len(weights_and_bias_from_last_layer)}")
print(f"type of weights_and_bias_from_last_layer: {type(weights_and_bias_from_last_layer)}")
print(f"Sample of weights_and_bias_from_last_layer: {weights_and_bias_from_last_layer[:10]}")


# %% 
model_for_decision_tree = Model(inputs=model.input, outputs=model.layers[-1].output)

# %%
print(model_for_decision_tree.summary())

# %%
features_train = model_for_decision_tree.predict(X_train)

# %%
features_test = model_for_decision_tree.predict(X_test)

# %%
decision_tree_classifier = DecisionTreeClassifier(random_state=42, max_depth=6)

# %%

decision_tree_classifier.fit(features_train, y_train)

# %%
predictions_from_decision_tree = decision_tree_classifier.predict(features_test)

# %%
# See the accuracy of the decision tree
from sklearn.metrics import accuracy_score

print("Accuracy on test set: {:.2f}%".format(accuracy_score(y_test, predictions_from_decision_tree)*100))

# %%
# Confusion matrix of Decision Tree for test dataset

cm = confusion_matrix(y_test, predictions_from_decision_tree)
print("Confusion matrix:\n", cm)
...
plt.figure(figsize=(10, 5))
sns.heatmap(cm, annot=True, fmt='g')
plt.title("Confusion Matrix for test Dataset on Decision Tree")
plt.xlabel("Predicted Label")
plt.ylabel("True Label")
plt.show()
...
# %%
# Classification report for test dataset for decision tree
print("Classification report (Decision Tree):\n", classification_report(y_test, predictions_from_decision_tree, target_names=['Normal (Class 0)','Pneumonia (Class 1)']))
# %%
